class ContainersDestroyService
  include Callable
  ...
  def call
    destroy_containers(@container_ids, @user_context, @request_path)
  end

  private

  def destroy_containers(container_ids, user_context, request_path)
    result_success = []
    result_errors = []
    jsonapi_error = nil
    container_ids.each_with_index do |container_id, idx|
      begin
        ...
        if ContainerPolicy.new(user_context, container).destroy?
          Searchkick.callbacks(:inline) do
            if (error = container.request_destroy(user_context.user))
              container.errors.add(:base, error)
            end
          end
        else
