class BatchPolicy < ApplicationPolicy
  def create?
    has_feature_in_org(CREATE_BATCH)
  end

  def index?
    has_feature_in_org(VIEW_BATCHES) || has_feature_in_any_lab(MANAGE_BATCHES_IN_LAB)
  end
  ...
  def can_link_aliquots?
    is_authorized_in_org(LINK_BATCH_TO_ALIQUOTS) || is_authorized_in_lab([ MANAGE_BATCHES_IN_LAB,
                                                                             MANAGE_CONTAINERS_IN_LAB ])
  end
