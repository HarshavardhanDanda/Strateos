...
class DataObjectSpectrophotometryPlateReader extends React.Component {
  ...
  constructor(props) {
    ...
    this.getWavelengthCount = this.getWavelengthCount.bind(this);
  ...
  initializeState(data) {
    const wavelengthCount = this.getWavelengthCount(data);
    const toggleList      = this.getToggleList(data, wavelengthCount);
    const groupedData     = this.getGroupedData(data, wavelengthCount);
  ...
  getWavelengthCount(data) {
    return _.uniqBy(data.readings, 'wavelength_index').length;
  }

  getId(reading, wavelengthCount) {
    return reading.wavelength_index + (reading.interval_index * wavelengthCount);
  }

  getToggleList(data, wavelengthCount) {
    const readings = data.readings;

    const result = {};

    readings.forEach((reading) => {
      const id = this.getId(reading, wavelengthCount);

      if (!result[id]) {
        const color      = PlateColors.getColorAsHex(id);
        const modePrefix = data.mode.slice(0, 3).toLocaleUpperCase();
        const label      = `${modePrefix} ${id + 1}`;
  ...
  //   }, ...
  // }
  getGroupedData(data, wavelengthCount) {
    const readings = data.readings;
    const result   = {};

    readings.forEach((reading) => {
      const id         = this.getId(reading, wavelengthCount);
      const well_index = this.ctypeHelper.robotWell(reading.well);
  ...
  getDataTableData() {
    ...
    _.forEach(this.props.data.readings, (r) => {
      const id = this.getId(r, this.state.wavelengthCount);

      if (id === this.state.toggleId) {
        result.push({ index: r.well, value: toPrecision(r.value, 3), reading: id });
      }
    });
